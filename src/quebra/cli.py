...
#
# This module ships inside the wheel, so `Path(__file__).parent` is site-packages once
# installed. Anchoring output there would write the append-only evidence tree into the
# Python installation. Anchoring on the caller's directory is also what makes
# `quebra run my_job.py` mean the obvious thing from any project.
#
# Behaviour from a checkout is unchanged: run from the repository root and `cwd/output` and
# `cwd/jobs/active` are the same directories the old `main.py` used.
def _output_root(explicit: str | None) -> Path:
    return Path(explicit).expanduser().resolve() if explicit else Path.cwd() / "output"


def _jobs_dir(name: str) -> Path:
    return Path.cwd() / "jobs" / name
...
def _print_job(job) -> None:
    from quebra.core.reference import ArtifactRef, LocalRef
...
def main() -> None:
    ...
    run_parser.add_argument(
        "--output-root",
        default=None,
        help="where run directories are written; defaults to ./output",
    )
    run_parser.add_argument(
        "--reuse-deps",
        action="store_true",
