...
def lookup_prior(
    ...
    if not pd.api.types.is_datetime64_any_dtype(src[time_col]):
        raise ValueError(
            f"Source column '{time_col}' is not datetime dtype after parsing"
        )
    ...
    if reference_time is None:
        if isinstance(main, Mapping):
            meta = (
                main.get("meta", {})
                if isinstance(main.get("meta", {}), Mapping)
                else {}
            )
            run_start_unix_s = meta.get("run_start_unix_s")
            if run_start_unix_s is None:
                raise ValueError(
                    "reference_time not provided and main mapping has no 'run_start_unix_s' in meta"
                )
            ...
            if time_col not in main.columns:
                raise ValueError(
                    f"Main DataFrame missing time column '{time_col}' to infer reference_time"
                )
            mdf = main.copy()
    ...
    if len(before) == 0:
        ...
        raise ValueError(
            f"No source rows found before {reference_time}. Earliest source timestamp: {earliest}"
        )
    ...
    if isinstance(main, Mapping):
        ...
        if "t_rel_s" not in out:
            raise KeyError(
                "lookup_prior requires main mapping to include 't_rel_s' (relative seconds)"
            )
