...
`bench/results/promotion_report.md` does that, from the calibration bench, and only four
...
def measure_all_asymptotic_sizes(
    *, n: int = ASYMPTOTIC_SIZE_N, seed: int = ASYMPTOTIC_SIZE_SEED
    ...
    import analyzers.checks.c1_lewis_robinson as _c1
    import analyzers.checks.c2_anderson_darling as _c2
    import analyzers.checks.cvm_cramer_von_mises as _cvm
...
def _divergence_levels(tie_df: pd.DataFrame, threshold: float) -> dict[int, float]:
    ...
    for n, sub in tie_df.groupby("n"):
        ...
        ordered = sub.assign(
            _order=sub["y_levels"].replace(0, 10**9)
...
def build_instrument_validation(
    ...
    *,
    ...
    tiers = [
        ...
        TierRow("C2 Anderson-Darling", 3, TIER_PARTIAL,
        ...
        TierRow(
            ...
            "reference/r_reference_values.csv serial_indep_global_statistic, seed 707 - "
...
def render_tier_table_markdown(data: InstrumentValidationData) -> str:
    ...
    lines = [
        ...
        "",
        "GENERATED by `bench/instrument_report.py` from the same typed artifact the four",
    ...
    lines += ["", "## Tier 2 detail - published values", "",
    ...
    lines += ["", "## Tier 4 detail - cross-implementation", "",
    ...
    for n, levels in sorted(data.divergence_levels.items()):
        ...
        if not np.isfinite(levels):
            ...
            lines.append(f"| {n} | {int(levels)} distinct values or fewer | {tie_shown} |")
    lines += [
        "",
