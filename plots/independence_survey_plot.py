...
_CLASS_NAME_OVERRIDES = {("c3_serial_copula", "r_copula", ""): "C3SerialCopulaSurveyPlot"}
...
class IndependenceSurveyOverviewPlot(BasePlot):
    ...
    def build_matplotlib(self, result: object, style: str = "default") -> plt.Figure:
        ...
        with theme.style_context(style):
            ...
            for row, key in enumerate(keys):
                for col, clock in enumerate(clocks):
                    ...
                    if col == 0:
                        ax.set_ylabel(
                            grid.label.replace(" (", "\n("), rotation=0,
                            ha="right", va="center", labelpad=8, **theme.LABEL_TEXT,
                        )
            ...
            fig.legend(
                handles=legend, loc="lower center", ncol=len(VERDICT_ORDER),
                frameon=False, **theme.LEGEND_TEXT,
            )
            fig.suptitle(
