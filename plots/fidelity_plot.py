...
class FidelityPlot(BasePlot):
    def build_matplotlib(self, result: FidelityResult, style: str = "default") -> plt.Figure:
        ...
        with plt.style.context(plt_style):
            ...
            if "t_rel_s" not in frame.columns:
                raise KeyError("FidelityPlot requires 't_rel_s' in result frame")
            t_h = frame["t_rel_s"].to_numpy(dtype=float) / 3600.0
            infidelity = np.clip(frame["infidelity"].to_numpy(dtype=float), 1e-16, None)
            fidelity_pct = _fidelity_percent_from_infidelity(infidelity)
            if "rabi_base_hz" in frame.columns:
                rabi_base_hz = float(np.median(frame["rabi_base_hz"].to_numpy(dtype=float)))
            else:
                rabi_base_hz = np.nan
            ...
            if np.isfinite(rabi_base_hz):
                ax_inf.set_title(f"Infidelity with decade guides (Rabi used={rabi_base_hz:.5e} Hz)")
            else:
