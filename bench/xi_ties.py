...
**This is a study, not a pipeline layer.** Nothing outside `bench/` may import it
(`tests/test_bench_isolation.py` enforces that). It writes
`bench/results/xi_tie_experiment.csv`; a figure that wants these numbers declares that CSV
as a Dataset and loads it through the pipeline, so the dependency runs through provenance
rather than around it.

Run:  python bench/xi_ties.py
"""
