...
from bench.arms import ARM_A, ARM_B, ARM_C, ARM_D, ARM_E
from bench.grid import ALPHA, N_GRID
...
DIRECTED_AT = {
    ...
    # CvM, promoted 2026-08-14. Scored against Arm D like C2: it is the same class of
...
def realised_censoring(size: pd.DataFrame) -> dict[float, float]:
    ...
    "0.25" arm realises 0.167-0.169 throughout. The report used to say 0.25 in 36 places.
    """
    return {
...
def check_agreement(
    ...
    Criterion 4 used to be asserted in prose and never scored. This scores it: for every
...
def verdicts(size: pd.DataFrame, power: pd.DataFrame) -> pd.DataFrame:
    ...
    for row_label in sorted(size["row"].unique()):
        ...
        # and the driver string used to print 0.999. That is the same practice the
...
def build(runtime_note: str = "") -> str:
    ...
    add("# Promotion report - the five checks against the calibration bench\n")
    add(
    ...
    add(
        "**Four checks are assessed, not five.** C3 (`copula::serialIndepTest`) needs R, "
    ...
    # Computed, not typed: these two ranges used to be hand-written ("44-90 null cells",
