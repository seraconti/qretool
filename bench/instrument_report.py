"""Write `bench/results/instrument_report.md` from the instrument validation artifact.

A writer, not a study. It lives in `bench/` for one reason: `bench/results/*.md` is this
repo's only tracked home for GENERATED markdown (the `!bench/results/*.md` rule in
...
Run:  python bench/instrument_report.py
"""
...
OUT = ROOT / "bench" / "results" / "instrument_report.md"


def main() -> None:
    tie_path = ROOT / "bench" / "results" / "xi_tie_experiment.csv"
    if not tie_path.exists():
        raise FileNotFoundError(
            f"{tie_path} is missing. Run `python bench/xi_ties.py` first - the report "
            "quotes its divergence numbers and must not invent them."
        )
    data = build_instrument_validation(
        pd.read_csv(ROOT / "reference" / "load_haul_dump.csv"),
        pd.read_csv(ROOT / "reference" / "load_haul_dump_published.csv"),
        pd.read_csv(ROOT / "reference" / "r_reference_inputs.csv"),
        pd.read_csv(ROOT / "reference" / "r_reference_values.csv"),
